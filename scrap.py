...
class Tree:
    ...
    def split(self, node: Node):
        ...
        if p is None:
            self.root = p = Node(data=[mid])
        else:
            p.children.remove(node)
            insort(p.data, mid)

        new_nodes = [Node(data=[(k, v)], parent=p) for k, v in node.data]
    ...
    def __rotate(y: Node, direction: int):
        ...
        new_node = Node(data=[yy], parent=y.parent)
    ...
    def absorb(self, p):
        ...
        while p is not None and p.isfull:
            ...
            left, right = (Node([(k, v)], parent=g) for k, v in p.data)
            p.data.pop(MIDDLE)
